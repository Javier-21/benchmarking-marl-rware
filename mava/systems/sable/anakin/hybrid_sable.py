...
# It contains a variant of rec_sable that loads the params from a ff_sable checkpoint.
...
from mava.utils.render import render_one_episode_rec_sable

if not hasattr(jax._src.config, 'enable_memories'):
    class FakeConfig:
        value = False
    jax._src.config.enable_memories = FakeConfig()
...
def run_experiment(_config: DictConfig) -> float:
    """Runs experiment."""
    _config.logger.system_name = "hybrid_sable"
    ...
    if config.arch.render:
        # Render an episode.
        abs_hs = get_init_hidden_state(config.network.net_config, 1)
        #abs_hs = tree.map(lambda x: x[jnp.newaxis], abs_hs)
        render_one_episode_rec_sable(config, best_params, sable_execution_fn, eval_env, abs_hs)
...
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}Hybrid Sable experiment completed{Style.RESET_ALL}")
    return eval_performance
